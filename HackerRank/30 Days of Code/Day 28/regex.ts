let n: number = /* parseInt(readLine()) */6;
...
let personsOrdered: string[] = [];

person.forEach(peer => {
    let name: string = peer.split(' ')[0],
        email: string = peer.split(' ')[1],
        reg: RegExp = /^.+@gmail\.com$/g,
        test: boolean = reg.test(email);
    console.log(`${name} ${email} ${test}`);
    if (test) personsOrdered.push(name);
})
...
personsOrdered.sort((a, b) => a.localeCompare(b))
